function E = DE_16(X,Nt,maxx,maxy)
% Output arguments:
% bestmem              : parameter vector with f_best solution
% EvalFO               : number of function evaluations
% Run DE minimization; strategy: % DE/rand/2 ;  ui = pm5 + F*(pm1 - pm2 + pm3 - pm4);
    rng('shuffle');
    Np=30; 
    %Nt=46; 
    D=Nt*2; F=0.58; Cr=0.1; 
    maxIter=200; k = 0;
...
%     X=[rx,ry];
    ...
    while (k < maxIter)
        ...
        u = pm5 + F*(pm1 - pm2 + pm3 - pm4);  % differential variation
...
%     figure
%     plot(best(1,1:Nt),best(1,Nt+1:end),'ms','LineWidth',3)
%     grid on
% %     figure
    plot(CONVERGE)
