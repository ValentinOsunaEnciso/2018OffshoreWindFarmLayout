function E = DE_12(X,Nt,maxx,maxy)
% Output arguments:
% bestmem              : parameter vector with f_best solution
% EvalFO               : number of function evaluations
% Run DE minimization; strategy: DE/best/1 ; u = bm + F*(pm1 - pm2);
    rng('shuffle');
    Np=30; 
    %Nt=46; 
    D=Nt*2; F=0.38; Cr=0.5; 
    maxIter=200; k = 0;
...
%     X=[rx,ry];
    ...
    while (k < maxIter)
        ...
        u = bm + F*(pm1 - pm2);        % differential variation
...
%     figure
%     plot(best(1,1:Nt),best(1,Nt+1:end),'rs','LineWidth',3)
%     grid on
%     figure
%     plot(CONVERGE),hold on
    E=CaseC(Nt,maxx,maxy,best(1,1:Nt),best(1,Nt+1:end));
    plot(best(1,1:Nt),best(1,Nt+1:end),'*r')
end
